...
RESULT_AMPLITUDE_MOD amplitude_modulation_setup(AMPLITUDE_MODULATION * c,
    ...
    if (rate_hz < AMPLITUDE_MOD_MIN_RATE ||
        rate_hz > AMPLITUDE_MOD_MAX_RATE) {
        return AMPLITUDE_MOD_INVALID_RATE;
    }

    if (depth < AMPLITUDE_MOD_MIN_DEPTH ||
...
void    amplitude_modulation_read(AMPLITUDE_MODULATION * c,
                                 float * audio_in,
                                 float * audio_out,
                                 float * ext_mod,
    ...
    switch (c->type) {
        case AMP_MOD_SIN:
            for (int i=0;i<audio_block_size;i++) {
                trem_factor = 1.0 - (depth*(0.5*oscillator_sine(t+=inc)+0.5));
        ...
        case AMP_MOD_TRI:
            for (int i=0;i<audio_block_size;i++) {
                trem_factor = 1.0 - (depth*(0.5*oscillator_triangle(t+=inc)+0.5));
                audio_out[i] = audio_in[i]*trem_factor;
            }
            break;
        ...
        case AMP_MOD_SQR:
            for (int i=0;i<audio_block_size;i++) {
                trem_factor = 1.0 - (depth*(0.5*oscillator_square(t+=inc)+0.5));
                audio_out[i] = audio_in[i]*trem_factor;
            }
            break;
        ...
        case AMP_MOD_RAMP:
            for (int i=0;i<audio_block_size;i++) {
                trem_factor = 1.0 - (depth*(0.5*oscillator_ramp(t+=inc)+0.5));
                audio_out[i] = audio_in[i]*trem_factor;
            }
            break;
