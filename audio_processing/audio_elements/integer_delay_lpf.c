 ...
 */
RESULT_DELAY    delay_setup(DELAY_LPF * c,
                            float * delay_buffer,
                            uint32_t delay_buffer_size,
                            uint32_t delay_initial_length,
                            float feedback,
                            float feedthrough,
                            float a_coeff) {
    ...
    if (feedback < DELAY_MIN_FEEDBACK ||
    ...
    if (a_coeff != 0.0 && (a_coeff > DELAY_MAX_ACOEFF ||
        a_coeff < DELAY_MIN_ACOEFF)) {
 ...
 * @return Delay result (enumeration)
 */
RESULT_DELAY    delay_modify_dampening(DELAY_LPF * c,
                                      float coeff_new) {
 ...
 * @return Delay result (enumeration)
 */
RESULT_DELAY    delay_modify_length(DELAY_LPF * c,
                                    uint32_t delay_length_new)
{
    ...
    c->read_tap_inc = (float) ((int32_t) delay_length - c->read_tap) * (1.0/DELAY_LPF_LENGTH_TRANS_STEPS);
