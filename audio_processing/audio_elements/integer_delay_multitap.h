...
RESULT_MT_DELAY    multitap_delay_setup(MULTITAP_DELAY * c,
                                        float * delay_line,
                                        uint32_t delay_line_size,
                                        uint32_t num_taps,
...
RESULT_MT_DELAY multitap_delay_modify_taps(MULTITAP_DELAY * c, uint32_t * new_tap_offsets);

void    multitap_delay_read(MULTITAP_DELAY * c,
                   float * audio_in,
                   float * audio_out,
                   uint32_t audio_block_size);

void    multitap_delay_read_isolated(MULTITAP_DELAY * c,
                   float * audio_in,
                   float ** audio_outs,
                   uint32_t audio_block_size);
