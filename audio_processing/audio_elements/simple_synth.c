 ...
 */
RESULT_SIMPLE_SYNTH synth_setup(SIMPLE_SYNTH * c,
                                uint32_t attack,
                                uint32_t decay,
                                uint32_t sustain,
                                uint32_t release,
...
void    synth_read(SIMPLE_SYNTH * c, 
                   float * audio_out, 
    ...
    switch (c->synth_operator) {
        ...
        case SYNTH_PULSE:
            for (i=0; i<audio_block_size;i++) {
                audio_out[i] = vol * get_envelope(c) * oscillator_pulse(t, c->operator_param1);
                t+=t_inc;
                if (t >= 1.0) t -= 1.0;
                c->position++;
            }
 ...
 */
void    synth_play_note(SIMPLE_SYNTH * c,
                        uint32_t note,
                        float volume) {
 ...
 * @param volume Volume of the note (0.0->1.0 typically)
 */
void    synth_play_note_freq(SIMPLE_SYNTH * c,
                             float freq,
                             float volume) {
 ...
 */
void    synth_update_note_freq(SIMPLE_SYNTH * c,
                               float freq) {
...
static float get_envelope(SIMPLE_SYNTH * c) {
    ...
    // Release 
    if  (c->position < c->env_attack + c->env_decay + c->env_sustain + c->env_release) {
        uint32_t pos = c->position - (c->env_attack + c->env_decay + c->env_sustain);
