 ...
 */
RESULT_TUBE_DISTORTION    tube_distortion_setup(TUBE_DISTORTION * c,
                                                float drive,
                                                float gain,
                                                float contour,
    ...
    if (gain < TUBE_DISTORTION_GAIN_MIN || 
        gain > TUBE_DISTORTION_GAIN_MAX) {
        return TUBE_DISTORTION_INVALID_GAIN;
    }

    if (drive < TUBE_DISTORTION_DRIVE_MIN || 
        drive > TUBE_DISTORTION_DRIVE_MAX) {
        return TUBE_DISTORTION_INVALID_DRIVE;
    }
 
    filter_setup(&c->input_filter,
                BIQUAD_TYPE_BPF,
                BIQUAD_TRANS_MED,
                (pm float *) c->input_filter_coeffs,
                600.0,
                1.0,
                1.0,
                audio_sample_rate);

    filter_setup(&c->output_filter,
                BIQUAD_TYPE_BPF,
                BIQUAD_TRANS_MED,
                (pm float *) c->output_filter_coeffs,
                600.0+600.0*contour,
                1.5,
                1.0,
                audio_sample_rate);
 ...
 * @return Tube distortion result (enumeration)
 */
RESULT_TUBE_DISTORTION    tube_distortion_modify_gain(TUBE_DISTORTION * c, float gain_new) {
 ...
 * @return Tube distortion result (enumeration)
 */
RESULT_TUBE_DISTORTION    tube_distortion_modify_drive(TUBE_DISTORTION * c, float drive_new) {
 ...
 * @return Tube distortion result (enumeration)
 */
RESULT_TUBE_DISTORTION    tube_distortion_modify_threshold(TUBE_DISTORTION * c, float threshold_new) {
 ...
 * @return Tube distortion result (enumeration)
 */
RESULT_TUBE_DISTORTION    tube_distortion_modify_contour(TUBE_DISTORTION * c, float contour_new) {
 ...
 */
void    tube_distortion_read(TUBE_DISTORTION * c,
                             float * audio_in,
                             float * audio_out,
    ...
    float temp_audio_1[MAX_AUDIO_BLOCK_SIZE], temp_audio_2[MAX_AUDIO_BLOCK_SIZE];

    // Apply input filter
    filter_read(&c->input_filter,
                audio_in,
                temp_audio_1,
                audio_block_size);
