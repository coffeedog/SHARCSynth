 ...
 */
RESULT_GUITAR_SYNTH guitar_synth_setup(GUITAR_SYNTH * c,
                                       float clean_mix,
                                       float synth_mix,
    ...
    synth_setup(&c->synth_octave_low_1,
                c->synth_attack,
                c->synth_decay,
                c->synth_sustain,
                c->synth_release,
    ...
    synth_setup(&c->synth_octave_low_2,
                c->synth_attack,
                c->synth_decay,
                c->synth_sustain,
                c->synth_release,
    ...
    filter_setup(&c->env_filter,
                 BIQUAD_TYPE_BPF,
                 BIQUAD_TRANS_VERY_SLOW,
                 (pm float *) c->env_filter_coeffs,
                 400.0,
                 3.0,
                 1.0,
                 audio_sample_rate);
 ...
 */
void   guitar_synth_read(GUITAR_SYNTH * c,
                         float * audio_in,
                         float * audio_out,
    ...
    float synth_out_1[MAX_AUDIO_BLOCK_SIZE],
          synth_out_2[MAX_AUDIO_BLOCK_SIZE],
          synth_out_3[MAX_AUDIO_BLOCK_SIZE];


    c->current_lock = zero_crossing_read(&c->zc_detect, 
                                         audio_in, 
                                         audio_block_size,
                                         &c->detected_frequency);
    ...
    if (c->current_lock && !c->last_lock) {
        synth_play_note_freq(&c->synth,
                             c->detected_frequency,
                             c->synth_volume);
        synth_play_note_freq(&c->synth_octave_low_1,
                             c->detected_frequency*0.5,
                             c->synth_volume);
    ...
    synth_update_note_freq(&c->synth_octave_low_2, c->detected_frequency*0.25);
    ...
    for (int i=0;i<audio_block_size;i++) {
        ...
        audio_out[i] = (audio_in[i] * c->clean_mix *2.0) +
                       (synth_out_1[i]*0.5+synth_out_2[i]*0.95+synth_out_3[i]*0.5)*4.0*c->measured_ampitude*c->synth_mix;

    }

    // Apply envelope filter
    float env_freq = c->measured_ampitude*15000.0;
    if (env_freq > 800.0) env_freq = 800.0;

    filter_modify_freq(&c->env_filter, 400.0+env_freq);

    filter_read(&c->env_filter,
                audio_out,
                audio_out,
                audio_block_size);
