 ...
 */
RESULT_RING_MOD     ring_modulator_setup(RING_MODULATOR * c,
                                         float freq,
                                         float depth,
    ...
    if (freq > RING_MOD_FREQ_HZ_MAX ||
        freq < RING_MOD_FREQ_HZ_MIN) {
        return RING_MOD_INVALID_FREQ;
    }
    if (depth > RING_MOD_DEPTH_MAX ||
 ...
 * @return Ring modulator result (enumeration)
 */
RESULT_RING_MOD    ring_modulator_modify_freq(RING_MODULATOR * c, 
                                              float freq_new) {
 ...
 * @return Ring modulator result (enumeration)
 */
RESULT_RING_MOD    ring_modulator_modify_depth(RING_MODULATOR * c,
                                               float depth_new) {
 ...
 */
void    ring_modulator_read(RING_MODULATOR * c,
                            float * audio_in,
                            float * audio_out,
    ...
    for (int i=0;i<audio_block_size;i++) {
        audio_out[i] = (1.0-c->depth)*audio_in[i] + c->depth*audio_in[i]*oscillator_sine(c->t);
        c->t += c->t_inc;
    }
    c->t = c->t - floor(c->t);
